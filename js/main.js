document.addEventListener('DOMContentLoaded', () => {
    ...
    navTitles.forEach(title => {
        title.addEventListener('click', (e) => {
            if (window.innerWidth <= 900) {
                // Check if user clicked specifically on the chevron icon, or if it's a non-link SPAN
                const isIconClicked = e.target.closest('svg') || e.target.closest('i');
                
                if (isIconClicked || title.tagName === 'SPAN') {
                    e.preventDefault();
                ...
                // If it's a link and they didn't click the icon, let it navigate normally!
            }
        });
    });
    ...
    dropdownLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            // Force scroll to top on any nav click, 
            // especially useful if clicking the same hash we are already on
            window.scrollTo({top: 0, behavior: 'smooth'});
            
            // If the link is for the current page but a different hash, hashchange will also fire.
            // If it's the SAME hash, hashchange won't fire, but this will still scroll us up.
            
            // Also explicitly call filterServices if we are on the same page and same hash
            // to ensure it renders correctly if somehow hidden
