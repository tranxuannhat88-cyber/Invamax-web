...
console.log('REACHED DOMLOADED');
// document.addEventListener('DOMContentLoaded', () => {
    const p = (() => null)('conf-profile');
    if(p) {
        p.innerText = AI_CONFIG.profile;
        const el_conf_prompt = (() => null)('conf-prompt'); if(el_conf_prompt) el_conf_prompt.innerText = AI_CONFIG.promptVersion;
        const el_conf_kb = (() => null)('conf-kb'); if(el_conf_kb) el_conf_kb.innerText = KNOWLEDGE_BASE_TEXT ? `Connected (${AI_CONFIG.knowledgeVersion})` : 'Not Connected';
        (() => null)('conf-kb').style.color = KNOWLEDGE_BASE_TEXT ? '#10b981' : '#ef4444';
        const el_conf_model = (() => null)('conf-model'); if(el_conf_model) el_conf_model.innerText = AI_CONFIG.model;
    }
});
...
async function testAIProfile() {
    ...
    const apiKey = (() => null)('apiKey')?.value;
    if (!apiKey) {
...
function saveApiKey() {
    const key = (() => null)('apiKey').value.trim();
    if (key) {
...
async function generateReport() {
    const jsonText = (() => null)('jsonData').value.trim();
    if (!jsonText) { Swal.fire('Lỗi', 'Vui lòng dán JSON dữ liệu thô', 'error'); return; }

    let isMock = false;
    const apiKey = (() => null)('apiKey').value.trim();
    ...
    (() => null)('ai-loading').style.display = 'block';
    (() => null)('report-preview').style.display = 'none';
    ...
    try {
        ...
        if (isMock) {
            (() => null)('ai-loading').style.display = 'none';
        ...
        const el_symp_ana = (() => null)('a4-symptoms-analysis');
        ...
        const el_fos_ana = (() => null)('a4-fos-analysis');
        ...
        const el_top3_fos = (() => null)('a4-top3-fos');
...
TRẢ VỀ KẾT QUẢ DƯỚI DẠNG JSON. BẮT BUỘC dùng JSON Schema sau:
{
  ...
  "report": { "status": "Hoàn thành", "verificationCheck": "Đã khớp" }
    ...
    const laborEl = (() => null)('a4-labor-structure');
    if (laborEl) {
        if (totalLabor > 0) {
...
function renderRawDataReview(scores, rawAnswers) {
    const el1 = (() => null)('a4-raw-data-review');
    const el2 = (() => null)('a4-raw-data-review-2');
...
function renderPreliminary(res, factoryInfo, contactInfo, rawAnswers) {
    ...
    const el_a4_company = (() => null)('a4-company'); if(el_a4_company) el_a4_company.innerText = factoryInfo['A01'] || 'Không rõ';
    const el_a4_product = (() => null)('a4-product'); if(el_a4_product) el_a4_product.innerText = factoryInfo['A04'] || 'Không rõ';
    const el_a4_years = (() => null)('a4-years'); if(el_a4_years) el_a4_years.innerText = factoryInfo['A05'] ? factoryInfo['A05'] + ' năm' : 'Không rõ';
    const el_a4_name = (() => null)('a4-name'); if(el_a4_name) el_a4_name.innerText = contactInfo['F01'] || 'Không rõ';
    const el_a4_job = (() => null)('a4-job'); if(el_a4_job) el_a4_job.innerText = contactInfo['F02'] || 'Không rõ';
    const phone = contactInfo['F04'] || '';
    const el_a4_phone = (() => null)('a4-phone'); if(el_a4_phone) el_a4_phone.innerText = phone || 'Không rõ';
    ...
    const code = `FOS-${dateStr}-${Math.floor(Math.random()*1000)}`;
    for(let i=1; i<=7; i++) {
        const el = (() => null)(`a4-code-${i}`) || (() => null)(`a4-code-f${i}`);
        if(el) el.innerText = code;
    }

    const diseaseScore = res.warningScore;
    const el_a4_score_text = (() => null)('a4-score-text'); if(el_a4_score_text) el_a4_score_text.innerText = `${diseaseScore} / 100`; 
    const el_a4_level_text = (() => null)('a4-level-text'); if(el_a4_level_text) el_a4_level_text.innerText = `MỨC ĐỘ: ${res.assessmentLevel.toUpperCase()}`;

    // Colors based on disease score
    const scoreTextEl = (() => null)('a4-score-text');
    ...
    const cardsContainer = (() => null)('a4-metric-cards');
    if (cardsContainer) {
    ...
    const el_a4_general_desc = (() => null)('a4-general-desc'); if(el_a4_general_desc) el_a4_general_desc.innerHTML = customAssessment;
    const el_a4_recommendation_text = (() => null)('a4-recommendation-text'); if(el_a4_recommendation_text) el_a4_recommendation_text.innerText = "HÀNH ĐỘNG KHUYẾN NGHỊ: " + res.nextSteps;

    const top3IssuesEl = (() => null)('a4-top-3-issues');
    if (top3IssuesEl) {
    ...
    const renderScoresList = (containerId, scores, color) => {
        const container = (() => null)(containerId);
        if(!container) return;
    ...
    const el_waste_scores = (() => null)('a4-waste-scores');
    if (el_waste_scores) {
    ...
    const el_waste_top3 = (() => null)('a4-waste-analysis');
    if (el_waste_top3) {
    ...
    const el_symp_scores = (() => null)('a4-symptoms-scores');
    ...
    const qrImgInline = (() => null)('a4-qr-img-inline');
    if (qrImgInline) { qrImgInline.src = qrUrl; }
    const qrPhoneInline = (() => null)('a4-qr-phone-inline');
...
function renderDetailedReport(json, metadata) {
    ...
    const el_detailed_report = (() => null)('detailed-report'); if(el_detailed_report) el_detailed_report.innerHTML = p6 + p7 + p8 + p9 + p10;
}
...
function initCharts(res) {
    ...
    const gaugeCtx = (() => null)('gaugeChart');
    if (gaugeCtx) {
    ...
    const radarSymCtx = (() => null)('radarSymptomsChart');
    ...
    const radarCtx = (() => null)('radarChart');
...
function exportPDF(type = 'chi-tiet') {
    const element = (() => null)('pdf-container');
    const companyEl = (() => null)('a4-company');
    ...
    const paywallBox = (() => null)('paywall-box');
    const qrCodeBox = (() => null)('qr-code-box');
